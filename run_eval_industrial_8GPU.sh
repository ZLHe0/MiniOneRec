#!/bin/bash
# Evaluation script for Industrial_and_Scientific, configured for 8 GPUs (0-7)

export CUDA_HOME=/work/envs/MiniOneRec

EXP_NAME=$1
if [ -z "$EXP_NAME" ]; then
    echo "Usage: bash run_eval_industrial_8GPU.sh <model_path>"
    echo "  e.g. bash run_eval_industrial_8GPU.sh ./output/rl_position_industrial_qwen2.5-1.5b-instruct"
    exit 1
fi
