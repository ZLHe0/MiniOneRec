#!/bin/bash
# Evaluation script for Industrial_and_Scientific, configured for 4 GPUs (3,4,5,6)

EXP_NAME=$1
if [ -z "$EXP_NAME" ]; then
    echo "Usage: bash run_eval_industrial.sh <model_path>"
    echo "  e.g. bash run_eval_industrial.sh ./output/sft_qwen2.5-1.5b-instruct"
    exit 1
fi
